			...
			<div id="all-events">
				...
				@foreach($events as $event)
					<div class="event card" style="padding-left: 0.4rem;margin-bottom: 0.5rem;">
						<h2 class="event-date"> -)<a href="{{ route('event',['id' => $event->id]) }}"> {{ $event->event_name }} </a> <span class="event_going" style="float: right;"> <button class="btn btn-info"> Attend </button> </span></h2>
						<center>
						...
						<button class="btn btn-default btn-lg verify" verified='0' id="event_{{ $event->id }}"> Verify </button>
					</div>
				@endforeach
			</div>
		</div>
