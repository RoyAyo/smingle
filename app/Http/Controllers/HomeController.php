...
class HomeController extends Controller
{
    ...
    {
        if (auth()->user()->cluster == '0') {
            Session::flash('cluster_error','Registration Error!!,Please go and change profile settings, resubmit and answer the questions that follow');
        }

        $user_id = auth()->user()->id;

 
        $Notifications = Notifications::where('involved_id',$user_id)->orderBy('created_at','desc')->paginate(8);
